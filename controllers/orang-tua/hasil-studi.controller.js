...
export const getKkn = async (req, res, next) => {
    const responseBuilder = new ResponseBuilder(res)
    const npm = req.params.npm ? req.params.npm : null;

    if (!npm) {
        throw new NotFoundError("NPM tidak diberikan")
    }
    ...
    try {
        const data = await hasilStudiService.getKkn(mahasiswa.id)

        responseBuilder
            .status('success')
            .code(200)
            .message("berhasil mengambil data kkn")
            .json(data)
    } catch (error) {
        next(error)
    }
