...
export const exportExcelManajemenCPL = async (req, res, next) => {
    try {
        const { obeId } = req.params;
        const data = await obeService.getManajemenCplByObeId(obeId);

        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet('Manajemen CPL');

        // 1. Setup Kolom Tabel Excel
        worksheet.columns = [
            { header: 'Kode CPL', key: 'kode', width: 12 },
            { header: 'Deskripsi CPL', key: 'deskripsi', width: 60 },
            { header: 'Deskripsi CPL (Inggris)', key: 'deskripsiEn', width: 60 },
            { header: 'Target CPL', key: 'targetCpl', width: 15 },
            { header: 'Kategori', key: 'kategori', width: 20 },
            { header: 'Kode CPL Parent', key: 'parent', width: 20 }, // Tambahan kolom baru
        ];

        // 2. Styling Header (Warna Selang-Seling Sesuai Gambar)
        const headerRow = worksheet.getRow(1);
        headerRow.eachCell((cell, colNumber) => {
            cell.font = { bold: true, color: { argb: 'FF000000' } }; // Teks Hitam
            cell.alignment = { vertical: 'middle', horizontal: 'left' };
            cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } };

            // Logika Warna Spesifik: 
            // Kolom 1 (Kode), 2 (Deskripsi), dan 4 (Target) = Hijau (#00a65a)
            // Sisanya (3, 5, 6) = Orange (#f39c12)
        ...
        data.dataCpl.forEach((cpl) => {
            const row = worksheet.addRow({
                kode: cpl.kode || '-',
                deskripsi: cpl.deskripsi || '-',
                deskripsiEn: cpl.deskripsiEn || '-',
                targetCpl: cpl.targetCpl || '',
                // Sesuai gambar, kategori isinya singkatan (S, P, KU, KK). 
                // Kalau di DB simpannya panjang, Frontend/Backend tinggal sesuaikan.
                kategori: cpl.kategori || '-',
                parent: '-' // Default kosong, karena di schema DB sebelumnya belum ada relasi parent CPL
            });

            // 4. Styling per Baris Data (Biar bisa turun ke bawah/Wrap Text)
            row.eachCell((cell) => {
                cell.alignment = { vertical: 'top', wrapText: true };
                cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } };
            });
        ...
        // 5. Download Otomatis
        res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        res.setHeader('Content-Disposition', 'attachment; filename="Laporan_Manajemen_CPL.xlsx"');
        await workbook.xlsx.write(res);
        res.status(200).end();
    } catch (error) {
        next(error);
